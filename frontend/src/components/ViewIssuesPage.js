    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={handleSummarize} disabled={isAiLoading || !issues.length} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {issues.map(issue => (
                        <div key={issue.id} className="bg-gray-800 rounded-lg shadow p-6 flex flex-col justify-between">
                            <div>
                                <span className="inline-block bg-indigo-500 text-white text-xs font-semibold px-2 py-1 rounded-full uppercase mb-2">{issue.category}</span>
